...
public sealed class DynamicLeakAnalyzer : DiagnosticAnalyzer
{
    ...
    public override void Initialize(AnalysisContext context)
    {
        ...
        context.RegisterOperationAction(AnalyzeDynamicInvocationArguments, OperationKind.DynamicInvocation);
    ...
    private static void AnalyzeDynamicInvocationArguments(OperationAnalysisContext context)
    {
        if (ShouldSkip(context))
            return;
        ...
        var symbolInfo = semanticModel.GetSymbolInfo(invocationSyntax, context.CancellationToken);
    ...
    private static bool ShouldSkip(OperationAnalysisContext context)
    {
        ...
        if (!GetBoolOption(context, OptionAnalyzeGeneratedCode, defaultValue: false) &&
            IsGeneratedCode(context.Operation.Syntax.SyntaxTree))
            return true;
    ...
    {
        var tree = context.Operation.Syntax.SyntaxTree;
